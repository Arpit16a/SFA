...
function OptimizationForm({state='Maharastra'}) {
  ...
  return (
    <Paper elevation={3} style={{ padding: '20px', marginTop: '20px' }}>
      ...
      {Object.entries(optimizedRoutes).map(([crop, routes]) => (
        <Accordion key={crop}>
          ...
          <AccordionDetails>
            <Table>
              ...
              <TableBody>
                {Array.isArray(routes.redistribution) ? routes.redistribution.map((route, index) => route.from==state || route.to== state ?  (
                  <TableRow key={index}>
                    <TableCell>{route.from}</TableCell>
                    <TableCell>{route.to}</TableCell>
                    <TableCell>{route.quantity.toFixed(2)}</TableCell>
                    <TableCell>${route.cost.toFixed(2)}</TableCell>
                    <TableCell>
                      <RadioGroup
                        row
                        value={truckTypes[`${crop}-${index}`]}
                        onChange={(e) => handleTruckTypeChange(crop, index, e.target.value)}
                      >
                    ...
                    <TableCell>
                      {route.from == state &&(<Button 
                        onClick={() => handleCropsSendReceive(route.requestId, route, 'send', crop, index, truckTypes)} 
                        variant="contained" 
                        color="primary"
                      ...
                      {route.to ==state &&(<Button 
                        onClick={() => handleCropsSendReceive(route.requestId, route, 'receive', crop, index, truckTypes)} 
                        variant="contained" 
                        color="secondary"
                      >
                        Receive
                  ...
                  <TableRow>
                    <TableCell colSpan={7}>No routes available</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
